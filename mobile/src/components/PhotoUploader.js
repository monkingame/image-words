...
export default class PhotoUploader extends React.Component {
  ...
  options = {
    title: this.props.pickerTitle || '选择图片',
    storageOptions: {
      skipBackup: true,
      path: 'images'
    },
  ...
  openImagePicker = () => {
    ...
    ImagePicker.showImagePicker(this.options, async response => {
      // console.log('PhotoUploader openImagePicker response = ', response);
      ...
      // console.log('Uploader openImagePicker : ', filePath, photoData.length);
      ...
      //NOTE:photoData是缩小后的图，response.data是原始尺寸的图像
      if (this.props.onPhotoSelect) {
        // this.props.onPhotoSelect(photoData);
        this.props.onPhotoSelect(`data:${response.type};base64,${photoData}`);
        // this.props.onPhotoSelect(`data:${response.type};base64,${response.data}`);
